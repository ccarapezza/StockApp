...
<body>
	...
	<div id="wrap">
		...
		if(isset($_POST['RegistrarCompra'])){
			...
			$total			= $Vendedor->FormatoSql($TotalNeto['deudatotal']);
			$fecha			= FechaActual();
			$hora			= HoraActual();

			$facturaSqlQuery="INSERT INTO `factura` (`total`, `fecha`, `hora`, `usuario`, `cliente`, `tipo`, `habilitado`) VALUES ('{$total}', '{$fecha}',  '{$hora}', '{$vendedor}', '{$cliente}', '{$tipo}', '1')";

			// Agrego los datos para generar la factura
			$facturaSql		= $db->SQL($facturaSqlQuery);
