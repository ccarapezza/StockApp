...
class Productos extends Conexion {
	...
	public function EliminarProducto(){
		...
		if(isset($_POST['EliminarProducto'])){
			...
			$Nombre = filter_var($_POST['nombre'], FILTER_SANITIZE_STRING);
			$Nombre = ucwords($Nombre);
			$EliminarProductoSql = $this->Conectar()->query("DELETE FROM `producto` WHERE `id` = '{$IdProducto}'");
			if($EliminarProductoSql == true){
				echo'
				<div class="alert alert-dismissible alert-success">
					<button type="button" class="close" data-dismiss="alert">&times;</button>
					<strong>&iexcl;Excelente</strong> El producto "'.$Nombre.'" ha sido eliminada con exito.
				</div>
				<meta http-equiv="refresh" content="5;url='.URLBASE.'productos"/>';
			}else{
				echo'
				<div class="alert alert-dismissible alert-danger">
					<button type="button" class="close" data-dismiss="alert">&times;</button>
					<strong>&iexcl;Oh no!</strong> A ocurrido un error al eliminar el producto "'.$Nombre.'", por favor intentalo de nuevo.
				</div>
				<meta http-equiv="refresh" content="5;url='.URLBASE.'productos"/>';
			}
		}
