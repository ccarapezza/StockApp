...
class Sistema extends Conexion {
	...
	public function AperturaCaja(){
		...
		if(isset($_POST['AperturaCaja'])){
			...
			$hora	= HoraActual();
			$Unix	= time();
			//$AperturaCajaQuerySql = "INSERT INTO `cajaregistros` (`monto`, `tipo`, `fecha`, `hora`, `Detalle`, `unix`) VALUES ('{$monto}', '{$tipo}', '{$fecha}', '{$hora}', 'Apertura de Caja', '{$Unix}')";
			$AperturaCajaQuerySql = "INSERT INTO `cajaregistros` (`monto`, `tipo`, `fecha`, `hora`, `detalle`) VALUES ('{$monto}', '{$tipo}', '{$fecha}', '{$hora}', 'Apertura de Caja')";
			$AperturaCajaQuery	= $this->Conectar()->query($AperturaCajaQuerySql);
	...
	public function CajaChica(){
		if(isset($_POST['CajaChicaOperacion'])):
			...
			$hora		= HoraActual();
			$Unix		= time();
			$CajaChicaRegistroQuery = $this->Conectar()->query("INSERT INTO `cajachicaregistros` (`monto`, `tipo`, `fecha`, `hora`, `detalle`) VALUES ('{$monto}', '{$tipo}', '{$fechaActual}', '{$hora}', '{$comentario}')");
			if($tipo == 0):
				$CajaChicaEntradaDineroQuery= $this->Conectar()->query("UPDATE `cajachica` SET `monto` = `monto`+'{$monto}' , `fecha` = '{$fechaActual}' , `hora` = '{$hora}' WHERE `id` = '1'");
			elseif($tipo==1):
				$CajaChicaSalidaDineroQuery	= $this->Conectar()->query("UPDATE `cajachica` SET `monto` = `monto`-'{$monto}' , `fecha` = '{$fechaActual}' , `hora` = '{$hora}' WHERE `id` = '1'");
			else:
			endif;
